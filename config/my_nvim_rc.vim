...
if g:isWindows
	...
	let $rc        = printf('%s/config/my_nvim_rc.vim', desktop)
	let $rce       = has('nvim') ? printf('%s/config/my_nvimworkenv.lua', desktop) : printf('%s/config/my_vimworkenv.vim', desktop)
	...
	let $vimFiles  = has('nvim') ? stdpath('data') : printf('%s/config/myVim', desktop)
...
packadd! matchit
packadd! cfilter
let loaded_netrwPlugin = 1 " do not load netrwet g:python3_host_prog = $homepath..'/Desktop/tools/python/python.exe'
...
function! GetInterestingParentDirectory()
	...
	elseif IsCoveredByRoslynLsp()
		let sln = GetSln()
		return fnamemodify(sln, isdirectory(sln) ? ':p' : ':p:h')
	else
		let dir = getcwd()
	endif
	return dir
...
function! IsCoveredByRoslynLsp()
	return luaeval('is_covered_by_roslyn_lsp()')
endfunction

function! HasRoslynLsp() abort
  for client in luaeval('vim.lsp.get_clients({ bufnr = vim.api.nvim_get_current_buf() })')
    if client.name ==# 'roslyn'
      return v:true
    endif
  endfor
  return v:false
...
function! JobStartExample(...)
	...
	let scratchbufnr = ResetScratchBuffer(g:rc.tmp.'/Job')
	echomsg "<start> ".cmd
	if g:isWindows
		let cmd = 'cmd /C '.cmd
	endif
	let s:job = jobstart(
		...
		\{
			\'cwd': getcwd(),
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('AppendToBuffer', [scratchbufnr]),
			\'on_exit':  { jobid,code,event-> execute('echomsg "[exit] '.code.'" | botright sbuffer '.scratchbufnr, '')}
		\}
	\)
...
function! AppendToBuffer(bufnr, job_id, data, event)
	if (!bufload(a:bufnr)) | call bufload(a:bufnr) | endif
	if empty(a:data) | return | endif
	let windowsEolData = a:data[0] =~ '\r$'
		\? map(a:data, {_, v -> substitute(v, '\r$', '', '')})
		\: a:data
	let currentLines = getbufline(a:bufnr, 1, '$')
	if (len(currentLines) == 1 && empty(currentLines[0]))
		call setbufline(a:bufnr, 1, windowsEolData)
		return
	end
	call appendbufline(a:bufnr, '$', windowsEolData)
endfunction
...
function! ResetScratchBuffer(pathOrNumber)
	...
	call setbufvar(bufnr, 'is_scratch_buffer', 1)
...
nnoremap <silent> <Leader>s :let buffers = get(w:, 'buffers', []) \| silent! split  \| let w:buffers = buffers<CR>
nnoremap <silent> <Leader>v :let buffers = get(w:, 'buffers', []) \| silent! vsplit \| let w:buffers = buffers<CR>
...
function! BrowseNext(next)
	...
	elseif luaeval('require("dap").session() ~= nil')
		if (a:next)
			lua require("dap").down()
		else
			lua require("dap").up()
		endif
	elseif luaeval('#vim.diagnostic.get(0, { severity = { min = vim.diagnostic.WARN } })') > 0
		call v:lua.vim.diagnostic.jump({"count": (a:next ? 1 : -1), "severity": { "min" : luaeval('vim.diagnostic.severity.WARN') }})
		call v:lua.vim.diagnostic.show()
	elseif &ft == 'fugitive'
	...
	elseif &ft == 'git'
		...
		if empty(quickfixbuffers)
			call v:lua.vim.diagnostic.jump({"count": (a:next ? 1 : -1) })
			call v:lua.vim.diagnostic.show()
			return
		endif
...
if has('nvim')
	augroup vimsourcing
		...
		autocmd FileType vim nnoremap <buffer> z! :FzfLua blines no_sort=true query=function!\ \|\ {{{<CR>
	augroup end
endif
...
function! Grep(qf_or_loclist, ...)
	...
	let s:job = jobstart(
		...
		\{
			...
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('AppendToBuffer', [scratchbufnr]),
			\'on_exit': function("GrepCB", [winnr, cmd, cwd, pattern, scratchbufnr, a:qf_or_loclist])
		\}
	\)
	call chanclose(s:job, 'stdin')
endfunction

function! GrepCB(winnr, cmd, cwd, pattern, scratchbufnr, qf_or_loclist, job, exit_status, event)
	if (a:exit_status)
	...
	"let lines = filter(getbufline(a:scratchbufnr, 1, '$'), 'v:val !~ "^\s*$"')
	let lines = getbufline(a:scratchbufnr, 1, '$')
	let g:g = lines
	call setbufline(a:scratchbufnr, 1, lines)
...
" Terminal: ---------------------------{{{
function! ConfigureFzfTerminalMappings()
	tunmap <buffer> <Esc>| tunmap <buffer> <C-U>| tunmap <buffer> <C-W>u
	tunmap <buffer> <C-J>
	tunmap <buffer> <C-K>
	tunmap <buffer> <C-N>| tunmap <buffer> <C-W><C-N>
	tunmap <buffer> <C-P>| tunmap <buffer> <C-W><C-P>
	tunmap <buffer> ^j
	tunmap <buffer> ^k
endfunction

function! ConfigureGlobalTerminalMappings()
	" Normal mode
	tnoremap <buffer> <silent> <Esc> <C-\><C-n>| tnoremap <buffer> <silent> <C-U> <Esc>| tnoremap <buffer> <silent> <C-W>u <C-U>
	" Moving
	tnoremap <buffer> <C-J> <Left>
	tnoremap <buffer> <C-K> <Right>
	tnoremap <buffer> <C-N> <Down>| tnoremap <buffer> <C-W><C-N> <C-N>
	tnoremap <buffer> <C-P> <Up>  | tnoremap <buffer> <C-W><C-P> <C-P>
	tnoremap <buffer> ^j <Home>
	tnoremap <buffer> ^k <End>
	" Editing
	tnoremap <buffer> <C-H> <BS>
	tnoremap <buffer> <C-L> <Del>
	tnoremap <buffer> <C-V> <C-\><C-N>"+pa
	tnoremap <buffer> <silent> <expr> <C-R> '<C-\><C-N>"'.nr2char(getchar()).'pi'
	" Window & Buffer management
	tnoremap <buffer> <silent> KK <C-\><C-N>:q<CR>
	tnoremap <buffer> <silent> <A-h> <C-\><C-N>:vert res -2<CR>i
	tnoremap <buffer> <silent> <A-l> <C-\><C-N>:vert res +2<CR>i
	tnoremap <buffer> <silent> <A-j> <C-\><C-N>res -2<CR>i
	tnoremap <buffer> <silent> <A-k> <C-\><C-N>res +2<CR>i
	tnoremap <buffer> <silent> <Leader>hh <C-\><C-n><C-W>h
	tnoremap <buffer> <silent> <Leader>jj <C-\><C-n><C-W>j
	tnoremap <buffer> <silent> <Leader>kk <C-\><C-n><C-W>k
	tnoremap <buffer> <silent> <Leader>ll <C-\><C-n><C-W>l
	tnoremap <buffer> <silent> <Leader><home><home> <C-\><C-n>1<C-W>W
	tnoremap <buffer> <silent> <Leader><end><end> <C-\><C-n>99<C-W>W
	tnoremap <buffer> <silent> <Leader>nn <C-\><C-N>gt
	tnoremap <buffer> <silent> <Leader>NN <C-\><C-N>:tabnew<CR>
	tnoremap <buffer> <silent> <Leader>pp <C-\><C-N>gT
	tnoremap <buffer> <silent> <Leader>xx <C-\><C-N>:tabclose<CR>
	tnoremap <buffer> <silent> <Leader>ss <C-\><C-N>:new<CR>
	tnoremap <buffer> <silent> <Leader>vv <C-\><C-N>:vnew<CR>
	tnoremap <buffer> <silent> <Leader>oo <C-\><C-N>:only<CR>i
	tnoremap <buffer> <silent> <Leader>OO <C-\><C-N>:exec 'tabnew \|b'.bufnr()<CR>
	tnoremap <buffer> <silent> <Leader>tt <C-\><C-N>:if &termwinsize == '' \| let &termwinsize='0*9999' \| else \| let &termwinsize='' \| endif<CR>
	tnoremap <buffer> <silent> <Leader>== <C-\><C-N>=
	tnoremap <buffer> <silent> HH <C-\><C-N>:CycleBackwards<CR>
	tnoremap <buffer> <silent> LL <C-\><C-N>:CycleForward<CR>
	" Query row
	tnoremap <buffer> <silent> <Leader>qq <C-\><C-N>:ToggleueryRow<CR>
	tnoremap <buffer> <silent> <Leader>qQ <C-\><C-N>:CreateQueryRow<CR>
endfunction

function! ConfigureFzfExcludedTerminalMappings()
	" Normal mode
	tnoremap <silent> <Esc> <C-\><C-n>| tnoremap <silent> <C-U> <Esc>| tnoremap <silent> <C-W>u <C-U>
	" Moving
	tnoremap <C-J> <Left>
	tnoremap <C-K> <Right>
	...
	" Window & Buffer management
	tnoremap <silent> KK <C-\><C-N>:q<CR>
	tnoremap <silent> <A-h> <C-\><C-N>:vert res -2<CR>i
	tnoremap <silent> <A-l> <C-\><C-N>:vert res +2<CR>i
	tnoremap <silent> <A-j> <C-\><C-N>res -2<CR>i
	tnoremap <silent> <A-k> <C-\><C-N>res +2<CR>i
	tnoremap <silent> <Leader>hh <C-\><C-n><C-W>h
	tnoremap <silent> <Leader>jj <C-\><C-n><C-W>j
	tnoremap <silent> <Leader>kk <C-\><C-n><C-W>k
	tnoremap <silent> <Leader>ll <C-\><C-n><C-W>l
	tnoremap <silent> <Leader><home><home> <C-\><C-n>1<C-W>W
	tnoremap <silent> <Leader><end><end> <C-\><C-n>99<C-W>W
	tnoremap <silent> <Leader>nn <C-\><C-N>gt
	tnoremap <silent> <Leader>NN <C-\><C-N>:tabnew<CR>
	tnoremap <silent> <Leader>pp <C-\><C-N>gT
	tnoremap <silent> <Leader>xx <C-\><C-N>:tabclose<CR>
	tnoremap <silent> <Leader>ss <C-\><C-N>:new<CR>
	tnoremap <silent> <Leader>vv <C-\><C-N>:vnew<CR>
	tnoremap <silent> <Leader>oo <C-\><C-N>:only<CR>i
	tnoremap <silent> <Leader>OO <C-\><C-N>:exec 'tabnew \|b'.bufnr()<CR>
	tnoremap <silent> <Leader>tt <C-\><C-N>:if &termwinsize == '' \| let &termwinsize='0*9999' \| else \| let &termwinsize='' \| endif<CR>
	tnoremap <silent> <Leader>== <C-\><C-N>=
	tnoremap <silent> HH <C-\><C-N>:CycleBackwards<CR>
	tnoremap <silent> LL <C-\><C-N>:CycleForward<CR>
	" Query row
	tnoremap <silent> <Leader>qq <C-\><C-N>:ToggleQueryRow<CR>
	tnoremap <silent> <Leader>qQ <C-\><C-N>:CreateQueryRow<CR>
endfunction

augroup terminal
	au!
	autocmd TermOpen * call ConfigureGlobalTerminalMappings()
	autocmd FileType fzf call ConfigureFzfTerminalMappings()
	autocmd TermOpen * if (getbufvar(bufnr(), 'is_scratch_buffer', 0) == 0) | startinsert | endif
augroup end

" Folding: ----------------------------{{{
vnoremap <silent> <space> <Esc>zE:let b:focus_mode=1 \| setlocal foldmethod=manual<CR>`<kzfgg`>jzfG`<
nnoremap <silent> <space> :if !has_key(b:, 'focus_mode') \| let b:focus_mode=len(filter(range(1, line('$')), 'foldclosed(v:val) != -1')) > 0 \| endif \| exec('normal! '.(b:focus_mode==1 ? 'zR' : 'zM')) \| let b:focus_mode=!b:focus_mode<CR>
...
command! -bar UnderlineCurrentSearchItem silent call matchadd('CurSearch', '\c\%#'.@/, 101)
...
" nvim has Lua create that mapping
lua vim.keymap.del("i", "<Tab>")
...
nnoremap <C-O> :JumpToPrevious<CR>
...
func! FuzzyFiles(...)
	let path = a:0 ? a:1 : ''
	call execute('FzfLua files cwd='..path)
endfunc
...
nnoremap q, :FzfLua history<CR>
nnoremap q; :FzfLua buffers<CR>
...
function! CopyPreviouslyYankedItemToCurrentDirectory()
	...
	if has('win32')
		...
		let s:job = jobstart(
			...
			\{
				\'cwd': cwd,
				\'stderr_buffered': v:true, 'on_stderr':   { chan,msg,_  -> execute("echomsg '".substitute(join(msg),"'","''","g")."'",  1) },
				\'on_exit':  function('RefreshBufferAndMoveToPath', [item_finalname])
			\}
		\)
	else
		let item = '/'.item
...
function! DeleteItemUnderCursor()
	...
	if has('win32')
		...
		let s:job = jobstart(
			...
			\{
				\'cwd': getcwd(),
				\'stdout_buffered': v:true, 'on_stderr':   { chan,msg,_  -> execute("echomsg '".substitute(join(msg),"'","''","g")."'",  1) },
				\'on_exit':  function('RefreshBufferAndMoveToPath')
			\}
		\)
	else
		let target = '/'.target
	...
	if has('win32')
		...
		let s:job = jobstart(
			...
			\{
				\'cwd': cwd,
				\'stdout_buffered': v:true, 'on_stderr':   { chan,msg,_  -> execute("echomsg '".substitute(join(msg),"'","''","g")."'",  1) },
				\'on_exit':  function('RefreshBufferAndMoveToPathAndCleanMovedBuffer', [bufnr, item_finalname])
			\}
		\)
	else
		let item = '/'.item
	...
	if has('win32')
		...
		let s:job = jobstart(
			...
			\{
				\'cwd': cwd,
				\'stdout_buffered': v:true, 'on_stderr':   { chan,msg,_  -> execute("echomsg '".substitute(join(msg),"'","''","g")."'",  1) },
				\'on_exit':  function('RefreshBufferAndMoveToPathAndCleanMovedBuffer', [bufnr, newname])
			\}
		\)
	else
	...
	if has('win32')
		...
		let s:job = jobstart(
			...
			\{
				\'cwd': cwd,
				\'stdout_buffered': v:true, 'on_stderr':   { chan,msg,_  -> execute("echomsg '".substitute(join(msg),"'","''","g")."'",  1) },
				\'on_exit':  function('RefreshBufferAndMoveToPath', [dirname])
			\}
		\)
	else
	...
	if has('win32')
		...
		let s:job = jobstart(
			...
			\{
				\'cwd': cwd,
				\'stdout_buffered': v:true, 'on_stderr':   { chan,msg,_  -> execute("echomsg '".substitute(join(msg),"'","''","g")."'",  1) },
				\'on_exit':  function('RefreshBufferAndMoveToPath', [filename])
			\}
		\)
	else
...
augroup my_dirvish
	...
	if g:isWindows
	autocmd FileType dirvish nnoremap <silent> <buffer> f :term<CR>
	autocmd FileType dirvish nnoremap <silent> <buffer> F :term powershell -NoLogo<CR>
elseif g:isWsl
	autocmd FileType =dirvish nnoremap <silent> <buffer> f :term<CR>
	autocmd FileType dirvish nnoremap <silent> <buffer> F :term<CR>
endif
...
function! WebBrowser(...)
	...
	let s:job= jobstart(cmd, {'cwd': g:rc.notes})
endfun
...
function! StartLiveNote()
	...
	sleep 2
	if (winnr('$')) == 1
		let viewingAreaWidth = float2nr(0.7 * &columns)
		let viewingAreaCmd = printf('leftabove %dvnew', viewingAreaWidth)
		call execute(viewingAreaCmd) | wincmd p
	endif
	ChangeOrToggleOpacity 40
...
function! FeedLiveNote()
	...
	for i in range(len(chunks))
		call chansend(b:livenote_job, chunks[i] . "\n")
	endfor
	call chansend(b:livenote_job, "<LiveNote>\n")
endfunction

function! StopLiveNote()
	autocmd! TextChangedI <buffer>
	call chansend(b:livenote_job, "\n\n")
	sleep 100m
	call jobstop(b:livenote_job)
endfunction
command! StopLiveNote call StopLiveNote()
command! ToggleLiveNote if has_key(b:, 'livenote_job') && jobwait([b:livenote_job], 0)[0] == -1 | call StopLiveNote() | else | call StartLiveNote() | endif
...
function! LocListToDirectory(dir, title)
	...
	if(&ft == 'qf')
		call matchadd('Conceal', '^[^/|]\+/')
		set conceallevel=3 concealcursor=nvic
		nnoremap <buffer> <silent> f :let f=GetQfListCurrentItemPath() \| lclose  \| call execute('FzfLua files cwd='.f)<CR>
		nnoremap <buffer> <silent> F :let f=GetQfListCurrentItemPath() \| call execute('FzfLua files cwd='.f)<CR>
	endif
endfunction
...
function! Meeting(...)
		...
		if (!has_key(b:, 'livenote_job') || jobwait([b:livenote_job], 0)[1] != -1)
			call StartLiveNote()
		endif
endfunc
...
function! CompileDiagramAndShowImage(inputFile, outputExtension, outputDir)
	...
	let s:job = jobstart(
		...
		\{
			...
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('AppendToBuffer', [scratchbufnr]),
			\'on_exit':  function('JobExitDiagramCompilationJob', [outputfile, scratchbufnr, a:inputFile])
		\}
	\)
endfunction
...
augroup markdown
	...
	autocmd FileType markdown nnoremap <buffer> z! :FzfLua blines no_sort=true query=^##<CR>
	autocmd FileType markdown nnoremap <buffer> Z! :FzfLua blines no_sort=tue query=[<CR>
...
function! StartPlantumlToSvg(diagram, diagramtype, array, pos)
	...
	let s:job = jobstart(
		...
		\{
			...
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('AppendToBuffer', [scratchbufnr]),
			\'on_stdin': function('AppendToBuffer', [plantumlbufnr]),
			\'on_exit':  function('StartPlantumlToSvgCB', [a:array, a:pos, scratchbufnr, plantumlbufnr])
		\}
	\)
endfunction
...
function! FindOrListNugets(...)
	...
	if empty(a:000) && &ft == 'cs'
		let cmd = printf('dotnet list "%s" package', GetSln())
		let cmd = 'cmd /C '.cmd
		echomsg cmd
		let s:job = jobstart(cmd, {
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('AppendToBuffer', [scratchbufnr]),
			\'on_exit': { jobid,code,event -> execute('sbuffer '.scratchbufnr)}})
		return
	endif
	...
	let s:job = jobstart(
		\cmd,
		\{
			\'stdout_buffered': v:true, 'on_stdout': function('OnSearchNugetOut', [scratchbufnr, found, tokens, sources]),
			\'stderr_buffered': v:true, 'on_stderr': function('OnSearchNugetErr', [scratchbufnr]),
			\'on_exit':  function('OnSearchNugetExit', [scratchbufnr, found, tokens])
		\}
	\)
	call chanclose(s:job, 'stdin')
endfunc
...
function! OnSearchNugetOut(scratchbufnr, foundNugets, searchTokens, sources, jobid, data, event)
	call AppendToBuffer(a:scratchbufnr, a:jobid, a:data, a:event)
	call AddNugetIfMatches(a:foundNugets, a:searchTokens, a:sources, channel, a:data)
endfunction

function! OnSearchNugetErr(scratchbufnr, jobid, data, event)
	call AppendToBuffer(a:scratchbufnr, a:jobid, a:data, a:event)
	call execute('echohl Constant | echomsg '''.substitute(join(a:data,''),"'","''","g").''' | echohl Normal',  1)
endfunction

function! OnSearchNugetExit(scratchbufnr, foundNugets, searchTokens, jobid, code, event)
	call execute('echomsg "[exit] '..'" | botright sbuffer '.scratchbufnr, '')
	call FindOrListNugetsExitCb(a:foundNugets, a:searchTokens, a:scratchbufnr, a:jobic, a:code, a:event)
endfunction

function! AddNugetIfMatches(foundNugets, searchTokens, sources, channel, data)
	let msg = join(a:data, '')
	if msg =~ '^source: '
...
function! FindOrListNugetsExitCb(foundNugets, tokens, scratchbufnr, job, code, event)
	if a:code != 0
		exec 'botright sbuffer' a:scratchbufnr
		return
	endif
...
function! Build()
	...
	let cmd = 'cmd /C dotnet build -v q --nologo /clp:NoSummary'
	let s:job = jobstart(
		\cmd,
		\{
			\'cwd': slnFolder,
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'on_exit':  function('BuildCB', [scratchbufnr])
		\}
	\)
	call chanclose(s:job, 'stdin')
endfunction
function! BuildCB(scratchbufnr, job, status, event)
	echomsg a:status ? 'Build failed :D' : 'Build succeeded :)'
	let lines = getbufline(a:scratchbufnr, 1, '$')
	let deduplicatedLines = uniq(sort(lines))
	if a:status || !empty(filter(deduplicatedLines, {_,x-> x =~ 'warning\|error'}))
		call setbufline(a:scratchbufnr, 1, deduplicatedLines)
...
"""	autocmd BufWrite *.cs,*.proto %s/^\(\s*\w\+\)\{0,6}\s\+class\s\+\zs\w\+\ze/\=uniq(sort(add(g:csClassesInChangedFiles, submatch(0))))/gne
	...
	autocmd FileType cs set efm=%E%f(%l\\\,%c):\ %trror\ %m\ [%.%#,%E%f(%l\\\,%c):\ %trror\ %m,%W%f(%l\\\,%c):\ %tarning\ %m\ [%.%#,%W%f(%l\\\,%c):\ %tarning\ %m,%Z%p^,%C%.%#,%-G%.%#
...
"""	autocmd FileType cs vnoremap <buffer> <silent> <Leader>W :OpenGitRepoHostCodeUrl<CR>
	...
	autocmd FileType cs nnoremap <buffer> <silent> <C-P> k[mkzz
	autocmd FileType cs nnoremap <buffer> <silent> <C-N> j]mkzz
	autocmd FileType cs nmap <buffer> <silent> z! :let g:lcd_qf = getcwd() \| lua document_methods_to_qf()<CR>
...
"""	autocmd FileType cs nmap <buffer> <LocalLeader>T <Plug>(omnisharp_run_tests_in_file)
	autocmd FileType cs nmap <silent> <buffer> <LocalLeader>U :call RunSlnTests('--filter FullyQualifiedName!~IntegrationTest')<CR>
	autocmd FileType cs nmap <silent> <buffer> <LocalLeader>I :call RunSlnTests('--filter FullyQualifiedName~IntegrationTest')<CR>
...
function RunSlnTests(opts)
	let scratchbufnr = ResetScratchBuffer(g:rc.tmp.'/Job')
	let sln = GetSln()
	let cmd = 'dotnet test ' .. sln .. ' --nologo ' .. a:opts
	execute 'sbuffer ' . scratchbufnr
	call termopen(
		\cmd,
		\{
			\'cwd': getcwd(),
...
function! RunScript(name, bang, ...)
	let cmd = printf('%s.bat %s', a:name, join(a:000, ' '))
	if empty(a:bang)
		if a:name =~ 'Start$'
			let s:job = jobstart(
				\cmd,
				\{
					\'cwd': getcwd(),
					\'on_exit':  { jobid,code,event-> execute('echomsg "[exit] '.code.'"', '')}
				\}
			\)
			call chanclose(s:job, 'stdin')
			quit
...
function! GetTermStatus(bufnr) abort
  if getbufvar(a:bufnr, '&buftype') !=# 'terminal'
    return ''
  endif

  let job = getbufvar(a:bufnr, 'terminal_job_id')
  return jobwait([job], 0)[0] == -1 ? 'running' : 'finished'
endfunction
...
function! LocListTerminalBuffers(bang)
	if empty(a:bang)
		...
		let terminalbuffers= map(filter(getbufinfo({'buflisted':1}), {_,x->getbufvar(x.bufnr, '&bt') == 'terminal'}), {_,x -> {'bufnr': x.bufnr, 'valid': 1, 'text': GetTermStatus(x.bufnr)}})
		...
		let terminalbuffers= map(filter(getbufinfo({'buflisted':1}), {_,x->getbufvar(x.bufnr, '&bt') == 'terminal' && stridx(GetTermStatus(x.bufnr), 'running') != -1 }), {_,x -> x.bufnr})
...
function! BuildReverseDependencyTree(...)
	...
	for i in range(len(slnprojs))
		...
		if isdirectory(csprojFolder.'/obj')
			...
			call add(jobs, jobstart(
				\cmd,
				\{
					\'on_stdout': function('GetProjectLastBuiltTimestampCB', [reverseDependencyTree[project]]),
					\'on_stderr':   { chan,msg,_  -> execute('echohl Constant | echomsg '''.substitute(join(msg),"'","''","g").''' | echohl Normal',  1) }
				\}
			\))
		endif
	endfor
...
function! GetProjectLastBuiltTimestampCB(reverseDependencyTreeItem, chan, msg, event)
	let a:reverseDependencyTreeItem.last_build_timestamp = str2nr(join(a:msg, '')) + 1
endfunction
...
function! VsTestCB(testedAssembly, csprojsWithNbOccurrences, scratchbufnr, sln, buildAndTestJobs, job, status, event)
	if a:status != 0
		exec 'botright sbuffer' a:scratchbufnr
		return
	endif
...
function! CascadeBuild(csproj, csprojsWithNbOccurrences, reverseDependencyTree, scratchbufnr, modifiedClasses, previouslyBuiltCsproj, sln, buildAndTestJobs)
	if a:csprojsWithNbOccurrences[a:csproj] > 1
		...
		call add(a:buildAndTestJobs, jobstart(
			\cmd,
			\{
				\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
				\'stderr_buffered': v:true, 'on_stderr': function('OnCascadeBuildErr', [scratchbufnr]),
				\'on_exit': function("CascadeReferences", [a:reverseDependencyTree[a:csproj].consumers, a:csprojsWithNbOccurrences, a:reverseDependencyTree, a:scratchbufnr, a:modifiedClasses, a:csproj, a:sln, a:buildAndTestJobs])
			\}
		\))
	endif
endfunction
...
"""todo
function! OnCascadeBuildErr(scratchbufnr, jobid, data, event)
	call execute('echohl Constant | echomsg '''.substitute(join(a:data, ''),"'","''","g").''' | echohl Normal',  1)
	call AppendToBuffer(a:scratchbufnr, a:jobid, a:data, a:event)
endfunction

function! CascadeReferences(csprojs, csprojsWithNbOccurrences, reverseDependencyTree, scratchbufnr, modifiedClasses, previouslyBuiltCsproj, sln, buildAndTestJobs, job, status, event)
	if a:status != 0
		exec 'botright sbuffer' a:scratchbufnr
		return
	endif
...
function! TestCsproj(path, csprojsWithNbOccurrences, scratchbufnr, modifiedClasses, sln, buildAndTestJobs)
	...
	call add(a:buildAndTestJobs, jobstart(
		...
		\{
			\'out_io': 'buffer',
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('OnTestCsprojErr', [scratchbufnr]),
			\'on_exit': function("VsTestCB", [assemblyToTest, a:csprojsWithNbOccurrences, a:scratchbufnr, a:sln, a:buildAndTestJobs])
		\}
	\))
endfunction

function! OnTestCsprojErr(scratchbufnr, jobid, data, event)
	call execute('echomsg '''.substitute(join(a:data, ''),"'","''","g").'''',  1)
	call AppendToBuffer(a:scratchbufnr, a:jobid, a:data, a:event)
endfunction
...
function! BuildTestCommit(all, resetCache, ...)
	...
	if a:all
		...
		if exists(':Rebuild'.slnFilename)
			...
			call add(buildAndTestJobs, jobstart(printf('MSBuild.exe -nologo -t:Clean -v:quiet "%s"', sln), {'stdout_buffered': v:true, 'on_exit': function('PostCleanCB', [sln, buildAndTestJobs])}))
		endif
	else
...
function! PostCleanCB(sln, buildAndTestJobs, ...)
	...
	call add(a:buildAndTestJobs, jobstart(printf('MSBuild.exe -nologo -t:Build -v:quiet "%s"', a:sln), {'stdout_buffered': v:true, 'on_exit': function('PostBuildCB', [a:sln, a:buildAndTestJobs])}))
	silent call BuildReverseDependencyTree(a:sln)
endfunction

function! PostBuildCB(sln, buildAndTestJobs, job, status, event)
	if a:status
...
function! GetCsprojsWithChanges(sln)
	...
	for project in projectsThatMightHaveChanges
		...
		call add(jobs, jobstart(cmd,{'on_stdout': function('GetProjectChangedCB', [csprojsWithChanges, project])}))
	endfor
...
function! RunQuery()
	...
	let s:job = jobstart(BuildCommandToRunAsJob(request), BuildQueryRowJobOptions(w:row, queryFilenameWithoutExtension))
endfunction
...
function! BuildQueryRowJobOptions(row, queryFilenameWithoutExtension)
	...
	return {
		\'cwd': getcwd(),
			\'stdout_buffered': v:true, 'on_stdout': function('AppendToBuffer', [scratchbufnr]),
			\'stderr_buffered': v:true, 'on_stderr': function('AppendToBuffer', [scratchbufnr]),
		\'on_exit':  function('DisplayQueryJobOutput', [scratchbufnr, historyBufDirvishDirValue, a:queryFilenameWithoutExtension, a:row.id])
	\}
endfunction

function! DisplayQueryJobOutput(bufnr, historyBufDirvishDirValue, queryFilenameWithoutExtension, rowId, channel, data, event)
...
if has('nvim')
	source $homepath/Desktop/config/my_nvim_init.lua
	let g:python3_host_prog = g:rc.desktop .. '/tools/python/python.exe'
endif
